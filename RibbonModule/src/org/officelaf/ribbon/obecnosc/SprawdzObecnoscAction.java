...
public class SprawdzObecnoscAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (top instanceof MainTopComponent) {
            maintop = (MainTopComponent) top;
        } else if (top instanceof GrupaCwiczeniowaTopComponent) {
            grouptop = (GrupaCwiczeniowaTopComponent) top;
        }
    ...
    private void openPresenceTableTopComponent(Termin term) {

        Mode mode = WindowManager.getDefault().findMode("editor");
        for (TopComponent tc : WindowManager.getDefault().getOpenedTopComponents(mode)) {
            if (tc instanceof PresenceTableTopComponent) {
                if (((PresenceTableTopComponent) tc).getTermId() == term.getId()) {
                    if (!tc.isOpened()) {
                        tc.open();
                    }
                    tc.requestActive();
        ...
        PresenceTableTopComponent ptop = new PresenceTableTopComponent();
